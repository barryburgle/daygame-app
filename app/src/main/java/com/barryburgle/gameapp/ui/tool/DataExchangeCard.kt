                ...
                Row(
                    ...
                    Column(
                        ...
                        FilenameComposable(
                            ...
                            localContext,
                            if (DataExchangeTypeEnum.EXPORT.type == cardTitle) {
                                state.exportSentPingsFileName
                            } else if (DataExchangeTypeEnum.IMPORT.type == cardTitle) {
                                state.importSentPingsFileName
                            } else "",
                            buttonFunction = {
                                importExportButtonFunction(
                                    cardTitle,
                                    ...
                                    onEvent,
                                    localContext,
                                    ToolEvent::SetAllSentPings
                                )
                            },
                            reloadFunction = {
                                if (DataExchangeTypeEnum.IMPORT.type == cardTitle) {
                                    onEvent(
                                        ToolEvent.SetImportSentPingsFileName(
                                            csvFindService.getLastFilenameInFolder(
                                                state.importFolder,
                                                "sent_ping"
                                            )
                        ...
                        FilenameComposable(
                            ...
                            reloadFunction = {
                                if (DataExchangeTypeEnum.IMPORT.type == cardTitle) {
                                    onEvent(
                                        ToolEvent.SetImportSettingsFileName(
                                            csvFindService.getLastFilenameInFolder(
                                                state.importFolder,
                                                "setting"
                                            )
                            ...
                            filenameOnEvent = {
                                if (DataExchangeTypeEnum.EXPORT.type == cardTitle) {
                                    onEvent(ToolEvent.SetExportSettingsFileName(it))
                                } else if (DataExchangeTypeEnum.IMPORT.type == cardTitle) {
                                    onEvent(ToolEvent.SetImportSettingsFileName(it))
                                }
                            }
                        )
                    }
