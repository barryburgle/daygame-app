...
fun CustomSummaryDialog(
    ...
    val summaryEntries = remember(customSummaryStartDate, customSummaryEndDate, state) {
        ...
        val datesByDate =
            state.allDates.filter { it.date != null }.groupBy { FormatService.parseDate(it.date!!) }
        ...
        val rawData = baseEntities.associateWith { entity ->
            ...
            val filteredSeries = series.filter { entry ->
                !entry.date.isBefore(customSummaryStartDate) && !entry.date.isAfter(
                    customSummaryEndDate
                )
            }
            val total =
                if (entity == HeatmapEntityEnum.INDEX) 0f else filteredSeries.sumOf { it.count.toDouble() }
                    .toFloat()
        ...
        baseEntities.forEach { entity ->
            ...
            when (entity) {
                ...
                HeatmapEntityEnum.DATES -> {
                    resultList.add(
                        SummaryEntryModel(
                            label = "Pull to Date Ratio",
                            total = 0f,
                            average = pullToDateRatioVal + " %",
                            isIndex = false,
                            isRatio = true
                        )
                    )
                ...
                HeatmapEntityEnum.PULLED -> {
                    resultList.add(
                        SummaryEntryModel(
                            label = "Bounce to Pull Ratio",
                            total = 0f,
                            average = bounceToPullRatioVal + " %",
                            isIndex = false,
                            isRatio = true
                        )
                    )
                ...
                HeatmapEntityEnum.BOUNCED -> {
                    resultList.add(
                        SummaryEntryModel(
                            label = "Kiss to Bounce Ratio",
                            total = 0f,
                            average = kissToBounceRatioVal + " %",
                            isIndex = false,
                            isRatio = true
                        )
                    )
                ...
                HeatmapEntityEnum.KISSED -> {
                    resultList.add(
                        SummaryEntryModel(
                            label = "Lay to Kiss Ratio",
                            total = 0f,
                            average = layToKissRatioVal + " %",
                            isIndex = false,
                            isRatio = true
                        )
                    )
    ...
    val reportText = buildString {
        append(
            "Custom Summary Report (${FormatService.getDate(customSummaryStartDate.toString() + "T00:00Z")} to ${
    ...
    FlowDialog(
        onDismissRequest = { onEvent(OutputEvent.SwitchShowCustomSummaryDialog) },
        onConfirm = { onEvent(OutputEvent.SwitchShowCustomSummaryDialog) },
        title = {
            Row(
                ...
                Column(
                    ...
                    LargeTitleText("Custom summary", true)
                    LittleBodyText(
                        "${FormatService.getDate(customSummaryStartDate.toString() + "T00:00Z")} to ${
                            FormatService.getDate(customSummaryEndDate.toString() + "T00:00Z")
                        }"
                    )
                }
                Column(
